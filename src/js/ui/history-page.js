const HistoryPage = (() => {
  ...
  function renderStatsSection() {
    ...
    const artigosEntries = Object.entries(
      statsData.artigosMaisConsultados || {},
    );
  ...
  function renderTable() {
    ...
    filteredHistory.forEach((entry) => {
      ...
      badge.textContent = entry.resultado
        ? entry.resultado.toUpperCase()
        : "N/A";
  ...
  function createHistoryItem(entry, metaNode) {
    ...
    const resultClass =
      entry.resultado === "inelegivel" ? "inelegivel" : "elegivel";
  ...
  function createStatCard(modifier, value, label) {
    ...
    card.className = modifier
      ? `stat-card history-card ${modifier}`
  ...
  function getSummaryIcon(key) {
    const icons = {
      total:
        '<svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7h18M3 12h18M3 17h18"/></svg>',
      inelegiveis:
