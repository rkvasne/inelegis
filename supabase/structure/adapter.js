...
const validScripts = ["db-structure", "tables", "functions", "rls-policies", "triggers", "diagnostico"];

if (!scriptType || !validScripts.includes(scriptType)) {
    console.error(`❌ Nome de script inválido. Use um de: ${validScripts.join(", ")}`);
    process.exit(1);
}

const hubPath = join(process.cwd(), ".agent", "hub", "system", "supabase-structure");
...
console.log(`🚀 [Inelegis Bridge] Executando lógica do Hub para: ${scriptType}...`);

const child = spawn("node", ["--preserve-symlinks", hubScript, "--outDir", localOutDir], {
    cwd: process.cwd(),
    env: childEnv,
    stdio: "inherit",
});
...
child.on("close", (code) => {
    if (code === 0) {
        ...
        console.error(`❌ [Bridge] O script do Hub encerrou com erro (código ${code}).`);
    }
    process.exit(code ?? 1);
});
