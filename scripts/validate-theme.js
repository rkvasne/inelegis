...
function loadCustomConfig() {
  if (options.config) {
    try {
      ...
      if (!options.quiet)
        console.log(
          `${colors.cyan}ℹ${colors.reset} Configuração carregada: ${options.config}`,
        );
    } catch (e) {
      console.error(
        `${colors.red}✗${colors.reset} Erro ao carregar config: ${e.message}`,
      );
    }
  }
  ...
  for (const file of configFiles) {
    if (fs.existsSync(file)) {
      try {
        ...
        if (!options.quiet)
          console.log(
            `${colors.cyan}ℹ${colors.reset} Configuração encontrada: ${file}`,
          );
        break;
      } catch (e) {
        /* ignore */
...
function shouldIgnoreFile(filePath) {
  ...
  if (options.ignore) {
    const ignoreRegex = new RegExp(
      options.ignore.replace(/\*\*/g, ".*").replace(/\*/g, "[^/]*"),
    );
  ...
  // Verificar --only
  if (options.only) {
    const onlyRegex = new RegExp(
      options.only.replace(/\*\*/g, ".*").replace(/\*/g, "[^/]*"),
    );
...
function checkHexColors(line, lineNum, file, fileType) {
  ...
  const hexPattern =
    /#([0-9a-fA-F]{3}|[0-9a-fA-F]{4}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})(?![0-9a-fA-F\w-])/g;
  ...
  while ((match = hexPattern.exec(line)) !== null) {
    ...
    if (/background/i.test(line))
      suggestion = "Use var(--bg-*) para backgrounds";
    else if (/color/i.test(line) && !/background/i.test(line))
      suggestion = "Use var(--text-*) para cores de texto";
    else if (/border/i.test(line))
      suggestion = "Use var(--border-*) para bordas";
    else if (/shadow/i.test(line))
      suggestion = "Use var(--shadow-*) para sombras";
...
function checkRgbColors(line, lineNum, file) {
  const rgbPattern =
    /rgba?\s*\(\s*(\d{1,3})\s*[,\s]\s*(\d{1,3})\s*[,\s]\s*(\d{1,3})(?:\s*[,\/]\s*[\d.%]+)?\s*\)/gi;
...
function checkHslColors(line, lineNum, file) {
  const hslPattern =
    /hsla?\s*\(\s*[\d.]+(?:deg|rad|grad|turn)?\s*[,\s]\s*[\d.]+%?\s*[,\s]\s*[\d.]+%?(?:\s*[,\/]\s*[\d.%]+)?\s*\)/gi;
...
function checkGradients(line, lineNum, file) {
  const gradientPattern =
    /(linear|radial|conic|repeating-linear|repeating-radial)-gradient\s*\([^)]+\)/gi;
...
function checkInlineStyles(line, lineNum, file) {
  ...
  while ((match = inlinePattern.exec(line)) !== null) {
    ...
    for (const color of CONFIG.namedColors.basic) {
      if (
        new RegExp(`(?:color|background)\\s*:\\s*${color}\\b`, "i").test(style)
      ) {
        addIssue(
          "inline-style",
          CONFIG.severityDefaults["inline-style"],
...
function checkVariableConsistency(content, file) {
  ...
  for (const varName of varsUsed) {
    const isSemantic = CONFIG.semanticVarPrefixes.some((prefix) =>
...
function checkMissingDarkMode(content, file) {
  ...
  const isMainCss = /styles?\.css$|main\.css$|global\.css$|app\.css$/i.test(
    file,
  );
  ...
  if (hasRootVars) {
    ...
    const hasDarkMode =
      /@media\s*\(\s*prefers-color-scheme\s*:\s*dark|\.dark\s*\{|\.dark-mode\s*\{|\[data-theme\s*=\s*["']dark/i.test(
        content,
      );
...
function checkLightColorsInDarkTheme(content, file) {
  ...
  const hasDarkTheme =
    /\.dark-theme\s*\{|@media\s*\(\s*prefers-color-scheme\s*:\s*dark/.test(
      content,
    );
  ...
  for (const colorVar of lightColorVars) {
    ...
    const rootDefPattern = new RegExp(
      `:root[^}]*${colorVar.replace(/[-]/g, "\\-")}\\s*:`,
      "i",
    );
    ...
    const darkOverridePattern = new RegExp(
      `\\.dark-theme[^}]*${colorVar.replace(/[-]/g, "\\-")}\\s*:|@media[^}]*prefers-color-scheme\\s*:\\s*dark[^}]*${colorVar.replace(/[-]/g, "\\-")}\\s*:`,
      "i",
    );
...
function checkInputsThemeProperties(content, file) {
  const inputSelectors = [
    {
      pattern: /\.form-input(?:\s*,\s*\.form-select)?\s*\{[^}]*\}/gi,
      names: ["form-input", "form-select"],
    },
    { pattern: /\.form-textarea\s*\{[^}]*\}/gi, names: ["form-textarea"] },
    {
      pattern:
        /input\[type=["']?(?:text|email|password|search|tel|url)["']?\]\s*\{[^}]*\}/gi,
      names: ["input"],
    },
  ...
  for (const { pattern, names } of inputSelectors) {
    ...
    if (matches) {
      for (const match of matches) {
        ...
        const usesThemeVars = themeVarPatterns.some((pattern) =>
          pattern.test(match),
        );
        ...
        const matchWithoutBgImage = match.replace(
          /background-image\s*:[^;]+;?/gi,
          "",
        );
        const hasHardcodedColor =
          /#[0-9a-fA-F]{3,8}/.test(matchWithoutBgImage) ||
          /rgba?\s*\([^)]+\)/.test(matchWithoutBgImage) ||
          /hsla?\s*\([^)]+\)/.test(matchWithoutBgImage) ||
          /\b(?:white|black|red|blue|green|yellow|gray|grey)\b/i.test(
            matchWithoutBgImage,
          );
        ...
        if (
          (hasBackground || hasColor || hasBorder) &&
          !usesThemeVars &&
          !hasHardcodedColor
        ) {
          const name = names[0];
...
function checkGradientThemeAdaptability(content, file) {
  ...
  while ((match = gradientPattern.exec(content)) !== null) {
    ...
    if (/^\s*--[\w-]+\s*:/.test(line)) {
      ...
      const hasDarkOverride = new RegExp(
        `\\.dark-theme[^}]*${varName}|@media[^}]*prefers-color-scheme\\s*:\\s*dark[^}]*${varName}`,
      ).test(content);
...
function checkActiveStateContrast(content, file) {
  ...
  for (const { pattern, name } of activePatterns) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      const usesVeryLight = veryLightColors.some((color) =>
...
function checkUnusedThemeVariables(content, file) {
  ...
  for (const varName of definedVars) {
    if (!usedVars.has(varName)) {
      ...
      if (
        !varName.match(/--(?:primary|success|warning|danger|info|neutral)-\d+/)
      ) {
...
function processFile(filePath) {
  ...
  try {
    ...
    if (!options.quiet) {
      console.error(
        `${colors.red}✗${colors.reset} Erro ao ler ${filePath}: ${error.message}`,
      );
    }
  }
}
...
function displayTextResults() {
  console.log("\n" + "═".repeat(70));
  console.log(
    `${colors.bold}📊 RESULTADOS DA VALIDAÇÃO DE TEMA${colors.reset}`,
  );
  console.log("═".repeat(70) + "\n");

  // Estatísticas gerais
  console.log(
    `${colors.cyan}📁 Arquivos verificados:${colors.reset} ${stats.filesScanned}`,
  );
  console.log(
    `${colors.cyan}📝 Linhas analisadas:${colors.reset} ${stats.linesScanned.toLocaleString()}`,
  );
  console.log(
    `${colors.cyan}🔍 Problemas encontrados:${colors.reset} ${stats.issuesFound}`,
  );
  console.log(`   ${colors.red}❌ Erros:${colors.reset} ${stats.errors}`);
  console.log(
    `   ${colors.yellow}⚠️  Warnings:${colors.reset} ${stats.warnings}`,
  );
  console.log(`   ${colors.blue}ℹ️  Info:${colors.reset} ${stats.info}`);

  if (stats.fixable > 0) {
    console.log(
      `   ${colors.green}🔧 Corrigíveis:${colors.reset} ${stats.fixable}`,
    );
  }
  ...
  if (issues.length > 0 && !options.quiet && !options.summary) {
    ...
    for (const [file, fileIssues] of Object.entries(byFile)) {
      ...
      for (const issue of fileIssues) {
        ...
        console.log(
          `   ${icon}${colors.reset} ${colors.dim}L${issue.line}:${colors.reset} ${issue.message}`,
        );
        ...
        if (options.fix && issue.suggestion) {
          console.log(
            `      ${colors.green}💡 ${issue.suggestion}${colors.reset}`,
          );
        }
      }
    }
  ...
  if (stats.errors === 0 && stats.warnings === 0) {
    console.log(
      `${colors.green}${colors.bold}✅ VALIDAÇÃO PASSOU${colors.reset}`,
    );
    console.log(
      `${colors.green}🎉 Todos os componentes usam variáveis de tema corretamente!${colors.reset}`,
    );
  } else if (stats.errors === 0) {
    console.log(
      `${colors.yellow}${colors.bold}⚠️  VALIDAÇÃO PASSOU COM WARNINGS${colors.reset}`,
    );
    console.log(
      `${colors.yellow}💡 Considere corrigir os ${stats.warnings} warning(s) para melhor manutenibilidade.${colors.reset}`,
    );
  } else {
    console.log(
    ...
    console.log(
      `${colors.red}💡 Corrija os ${stats.errors} erro(s) encontrado(s).${colors.reset}`,
    );
  }
...
function main() {
  ...
  if (!options.json && !options.quiet) {
    console.log(
      `${colors.bold}${colors.cyan}🎨 Theme Validator Pro v3.0.0${colors.reset}`,
    );
    console.log(
      `${colors.dim}Validador Universal de Temas CSS${colors.reset}\n`,
    );
    ...
    if (options.verbose) {
      console.log(
        `${colors.dim}Opções: ${JSON.stringify({
  ...
  if (options.json) {
    ...
    displayTextResults();
    console.log(
      `${colors.dim}⏱️  Tempo de execução: ${duration}ms${colors.reset}\n`,
    );
  }
