...
console.log(
  "Key:",
  SUPABASE_KEY ? SUPABASE_KEY.substring(0, 30) + "..." : "❌ NÃO CONFIGURADA",
);
...
async function testConnection() {
  try {
    ...
    const histResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/historico_consultas?select=id&limit=1`,
      {
        headers: {
          apikey: SUPABASE_KEY,
          Authorization: `Bearer ${SUPABASE_KEY}`,
    ...
    const rpcResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/rpc/verificar_elegibilidade`,
      {
        method: "POST",
        headers: {
          apikey: SUPABASE_KEY,
