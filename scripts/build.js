...
class Builder {
  ...
  async build() {
    ...
    this.log(`Iniciando build do Inelegis v${version}`, "info");
  ...
  async validateProject() {
    ...
    const requiredFiles = [paths.pages.index, paths.styles.main, paths.js.main];
    ...
    for (const file of requiredFiles) {
      if (!fs.existsSync(file)) {
        this.errors.push(
          `Arquivo obrigatório não encontrado: ${path.relative(this.projectRoot, file)}`,
        );
      }
    }
    ...
    for (const dir of requiredDirs) {
      if (!fs.existsSync(dir)) {
        this.warnings.push(
          `Diretório recomendado não encontrado: ${path.relative(this.projectRoot, dir)}`,
        );
      }
    }
  ...
  async validateHTML() {
    ...
    const checks = [
      ...
      },
      {
        ...
        message: "Idioma português definido",
      },
      {
    ...
    this.log(
      `HTML validado: ${passed}/${checks.length} verificações passaram`,
      "success",
    );
  }
  ...
  async validateCSS() {
    ...
    const checks = [
      ...
      {
        test: content.includes("--primary-"),
        message: "Paleta de cores primária",
      },
      ...
      { test: content.includes("@keyframes"), message: "Animações CSS" },
      {
        ...
        message: "Glassmorphism implementado",
      },
      {
        test: content.includes("@media"),
        message: "Responsividade implementada",
      },
    ...
    this.log(
      `CSS validado: ${passed}/${checks.length} verificações passaram`,
      "success",
    );
  }
  ...
  async validateJavaScript() {
    ...
    for (const jsPath of jsFiles) {
      if (!fs.existsSync(jsPath)) {
        this.errors.push(
          `Arquivo JS não encontrado: ${path.relative(this.projectRoot, jsPath)}`,
        );
        continue;
      }
  ...
  async validateData() {
    ...
    this.log(
      "Configuração do Supabase Client validada via Testes de Integração ✓",
      "success",
    );
  }
  ...
  generateReport() {
    ...
    console.log("\n" + "=".repeat(60));
    console.log(
      `📊 RELATÓRIO DE BUILD - INELEG-APP v${this.version || "0.0.0"}`,
    );
