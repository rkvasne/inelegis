...
class DocumentationAgent {
  ...
  checkRequiredDocs() {
    ...
    CONFIG.requiredRootDocs.forEach((required) => {
      const found = this.docs.find(
        (doc) => doc.name === required && doc.path === required,
      );
  ...
  generateRecommendations() {
    ...
    if (this.stats.total > 20) {
      recommendations.push(
        "Considerar consolidação - muitos documentos podem dificultar manutenção",
      );
    }
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log(
        'Use "node doc-agent.js help" para ver comandos disponíveis\n',
      );
      process.exit(1);
  }
}
