...
class DocumentationAgent {
  ...
  scanDirectory(dirPath, relativePath = "") {
    ...
    items.forEach((item) => {
      ...
      if (item === ".agent" || item === "node_modules" || item === ".git")
        return;
  ...
  checkRequiredDocs() {
    ...
    CONFIG.requiredRootDocs.forEach((reqDoc) => {
      ...
      if (!fs.existsSync(docPath)) {
        console.error(
          `❌ CRÍTICO: Documento obrigatório ausente na raiz: ${reqDoc}`,
        );
        this.stats.missing++;
...
async function main() {
  ...
  if (riskyDocs.length > 0) {
    ...
    riskyDocs.slice(0, 50).forEach((doc) => {
      // Limit to 50 to avoid flood
