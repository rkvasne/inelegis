...
function loadCustomConfig() {
    if (options.config) {
        try {
            ...
            if (customConfig.ignoreDirs) CONFIG.ignoreDirs = [...DEFAULT_CONFIG.ignoreDirs, ...customConfig.ignoreDirs];
            if (customConfig.ignoreFiles) CONFIG.ignoreFiles = [...DEFAULT_CONFIG.ignoreFiles, ...customConfig.ignoreFiles];
            if (customConfig.themeProperties) CONFIG.themeProperties = { ...DEFAULT_CONFIG.themeProperties, ...customConfig.themeProperties };
            if (customConfig.problematicPatterns) CONFIG.problematicPatterns = { ...DEFAULT_CONFIG.problematicPatterns, ...customConfig.problematicPatterns };
            if (customConfig.allowedContexts) {
                // Converter strings de regex de volta para RegExp se vierem do JSON
                const allowed = customConfig.allowedContexts.map(c => typeof c === 'string' ? new RegExp(c) : c);
                CONFIG.allowedContexts = [...DEFAULT_CONFIG.allowedContexts, ...allowed];
            }
        } catch (e) {
            console.error(`${colors.red}Erro ao carregar config: ${e.message}${colors.reset}`);
            process.exit(1);
        }
    }
}
...
function isIgnored(filePath) {
    ...
    if (options.ignore) {
        ...
        if (options.ignore.startsWith("*.")) return filename.endsWith(options.ignore.slice(1));
                ...
                if (supported) {
                    ...
                    if (options.only) {
                        if (options.only.startsWith("*.")) {
                            if (fullPath.endsWith(options.only.slice(1))) files.push(fullPath);
                        } else {
...
function checkHexColors(file, line, lineNum) {
    ...
    while ((match = hexRegex.exec(line)) !== null) {
        addIssue({
            ...
            context: line.trim(),
            suggestion: "Substitua por uma variável semântica (ex: var(--primary-500))"
        });
    }
}
...
function checkNamedColors(file, line, lineNum) {
    ...
    CONFIG.namedColors.basic.forEach(color => {
        ...
        while ((match = regex.exec(line)) !== null) {
            ...
            const isTailwindClass = /class(?:Name)?=["'][^"']*$/.test(line.substring(0, match.index));
...
function checkInlineStyles(file, line, lineNum) {
    const colorProps = ['color', 'background', 'border', 'fill', 'stroke', 'outline', 'shadow'];
    // Verifica se tem 'style=' e alguma propriedade de cor
    const hasStyle = line.includes('style=');
    const hasColorProp = colorProps.some(p => line.includes(p + ':') || line.includes(p + '-'));
    ...
    if (hasStyle && hasColorProp) {
        ...
        addIssue({
            ...
            line: lineNum,
            type: "inline-style",
            message: "Estilo inline detectado com definição de cor/visual (sem variável)",
...
function checkImportant(file, line, lineNum) {
    ...
    if (line.includes('!important')) {
        addIssue({
            ...
            context: line.trim(),
            suggestion: "Evite !important para manter a especificidade CSS gerenciável"
        });
    }
}
...
function addIssue(issue) {
    ...
    if (options.verbose) {
        ...
        console.log(`${color}[${issue.severity.toUpperCase()}] ${issue.file}:${issue.line} - ${issue.message}${colors.reset}`);
...
function main() {
    console.log(`${colors.bold}🔍 Theme Linter (Hub Core) Iniciado...${colors.reset}\n`);
    loadCustomConfig();

    const files = walkDir(process.cwd());
    console.log(`${colors.cyan}ℹ Encontrados ${files.length} arquivos para verificar.${colors.reset}\n`);
...
function printReport() {
    ...
    if (stats.issuesFound === 0) {
        console.log(`${colors.green}✅ Nenhum problema de tema encontrado!${colors.reset}`);
    } else {
        const statusColor = stats.errors > 0 ? colors.red : colors.yellow;
        console.log(`${statusColor}Validação concluída com problemas.${colors.reset}`);
    }
}
