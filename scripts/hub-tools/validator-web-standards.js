...
const CONFIG = {
  ...
  rules: {
    ...
    doctype: {
      ...
      msg: "Falta <!DOCTYPE html>",
    },
    lang: {
      required: true,
      pattern: /<html[^>]+lang=["'][^"']+["']/i,
      msg: "Falta atributo lang no <html>",
    },
    charset: {
      required: true,
      pattern: /<meta[^>]+charset=["'][^"']+["']/i,
      msg: "Falta charset UTF-8",
    },
    viewport: {
      required: true,
      pattern: /<meta[^>]+name=["']viewport["']/i,
      msg: "Falta meta viewport",
    },
    ...
    noHttp: {
      level: "error",
      check: (content) => !/http:\/\//.test(content),
      msg: "Links HTTP inseguros detectados (use HTTPS)",
    },
  },
};
    ...
    while ((match = regex.exec(content)) !== null) {
      ...
      if (link.startsWith("#")) {
        ...
        if (!this.hasId(content, id)) {
          this.addIssue({
            file: relFile,
            type: "link",
            message: `Âncora não encontrada: ${link}`,
            level: "error",
          });
      ...
      if (!fs.existsSync(targetPath)) {
        this.addIssue({
          file: relFile,
          type: "link",
          message: `Arquivo não encontrado: ${link}`,
          level: "error",
        });
        this.stats.linksBroken++;
      }
