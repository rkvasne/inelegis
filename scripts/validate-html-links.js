...
function normalizeTarget(htmlFile, rawUrl) {
  ...
  if (String(rawUrl).startsWith("#"))
    return { type: "anchor", value: String(rawUrl).slice(1) };
  if (url.startsWith("/"))
    return {
      type: "file",
      value: path.join(publicDir, url.replace(/^\//, "")),
    };
...
function main() {
  ...
  if (issues.length) {
    ...
    issues.slice(0, 10).forEach((it, i) => {
      console.log(
        `${i + 1}. ${path.relative(publicDir, it.file)} -> ${it.url} (${it.reason})`,
      );
    });
  }
