...
export const dashboardUI = {
    ...
    async loadKPIs() {
        ...
        if (data && data.length > 0) {
            const stats = data[0];
            document.getElementById('totalSearches').textContent = stats.total_searches.toLocaleString();
            document.getElementById('inelegiveisCount').textContent = stats.inelegiveis.toLocaleString();
            document.getElementById('elegiveisCount').textContent = stats.elegiveis.toLocaleString();
        }
    },
    ...
    async loadCharts() {
        // 1. Timeline Chart
        const { data: timelineData } = await window.supabase.from('analytics_timeline').select('*');
        if (timelineData) {
            const labels = timelineData.map(item => new Date(item.date).toLocaleDateString('pt-BR')).reverse();
        ...
        const { data: distData } = await window.supabase.from('analytics_result_distribution').select('*');
        if (distData) {
    ...
    async loadAuditLog() {
        ...
        if (data) {
            data.forEach(log => {
                ...
                const badgeClass = log.resultado === 'inelegivel' ? 'badge-danger' :
                    log.resultado === 'elegivel' ? 'badge-success' : 'badge-warning';
