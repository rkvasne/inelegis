...
|   9   | `20260226000200_hotfix_verificar_elegibilidade_normalizacao_paragrafo.sql` | Hotfix RPC base: normalização robusta de `p_paragrafo` (`caput`/`único`)  |
...
Execute os 9 arquivos em ordem (1 → 9).
...
2. **Funções:** `verificar_elegibilidade` e `verificar_elegibilidade_v2` (ver [interpretacao-tabela-oficial.md](../references/interpretacao-tabela-oficial.md))
   - A RPC base deve refletir o hotfix de normalização de `p_paragrafo` (migration 9).
