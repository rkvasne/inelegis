...
const mockDocument = {
  ...
  getElementById: (id) => {
    if (id === "toast-container") {
      return (
        mockDocument.body.children.find((c) => c.id === "toast-container") ||
        null
      );
    }
    return null;
...
test("Deve criar toast com o tipo correto", () => {
  ...
  assert.ok(
    toast.className.includes("toast--success"),
    "Deve ter classe toast--success",
  );
...
test("Deve escapar HTML na mensagem", () => {
  ...
  assert.ok(
    !toast.innerHTML.includes("<img"),
    "HTML não deve ser injetado diretamente",
  );
});
...
test("Deve expor mostrarToast global para compatibilidade", () => {
  assert.ok(
    ...
    "mostrarToast deve ser uma função",
  );
  ...
  assert.ok(
    ...
    "Deve converter tipo error corretamente",
  );
});
