...
const DATA_NORMALIZADO_PATH = path.join(__dirname, '../public/assets/js/data-normalizado.js');
...
test('data-normalizado.js não está vazio', () => {
    ...
    assert(content.length > 100, `Arquivo muito pequeno: ${content.length} bytes`);
});
...
test('data-normalizado.js é JavaScript válido', () => {
    ...
    const jsonMatch = content.match(/window\.__INELEG_NORMALIZADO__\s*=\s*(\[[\s\S]*?\]);/);
...
test('Registros têm estrutura esperada', () => {
    ...
    assert(Array.isArray(sample.estruturado.artigos), 'Campo "estruturado.artigos" não é array');
});
