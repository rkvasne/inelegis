...
function extrairArtigosCompletos(texto) {
    ...
    clausulas.forEach(clausula => {
        ...
        const regexArt = /(?:art\.?s?\.?\s+)([\d\w\-\.]+)(?:\s*(?:,|e|ou|c\/c)\s*([\d\w\-\.]+))?(?:\s*(?:,|e|ou|c\/c)\s*([\d\w\-\.]+))?/gi;
        let m;
        while ((m = regexArt.exec(clausula)) !== null) {
            ...
            const extractMulti = (ctx, single, plural, valuePattern) => {
                const allMarkers = (plural + "|" + single).split('|').sort((a, b) => b.length - a.length).map(m => m.replace('.', '\\.'));
                ...
                const clean = matchVal[1].split(/\s*(?:,|e|ou)\s*/).map(v => v.replace(/[º°ª'"'']/g, '').trim()).filter(v => v);
                return clean.length > 0 ? clean : [null];
            };

            const paragrafos = extractMulti(context, "§|parágrafo|paragrafo|par", "§§|parágrafos|paragrafos", "\\d+|único|unico");
            const incisos = extractMulti(context, "inciso|inc", "incisos|incs", "[ivxlcdm]+|\\d+");
            const alineas = extractMulti(context, "alínea|alinea|al", "alíneas|alineas|als", "['\"]?[a-z]['\"]?");

            const articlesFound = [m[1], m[2], m[3]].filter(v => v).map(v => v.replace(/\.$/, "").trim());
