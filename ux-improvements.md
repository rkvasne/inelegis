...
- [x] **T1.1**: Atualizar `styles.css` com novas utilidades de espaçamento e tipografia para os resultados.
- [x] **T1.2**: Criar estilos para os novos inputs da busca simples (grid responsivo).

### Fase 2: Refatoração da Busca Simples
- [x] **T2.1**: Alterar `public/consulta.html` para incluir campos de Parágrafo, Inciso e Alínea no `simple-mode-content`.
- [x] **T2.2**: Atualizar `ValidatorUI.js` para ler esses novos campos.
- [x] **T2.3**: Ajustar `ValidatorService.js` e a chamada RPC para passar todos os parâmetros.
- [x] **T2.4**: Melhorar `ValidatorUI.setupLeiSelect` para mostrar o nome completo da lei.

### Fase 3: Refinamento do Analisador de Sentença
- [x] **T3.1**: Atualizar `AnalyzerUI.js` para renderizar as linhas da tabela com o novo design (separando claramente Norma de Artigo).
- [x] **T3.2**: Ajustar a renderização do campo ASE para evitar o "encavalamento" de texto visto na imagem.
- [x] **T3.3**: Adicionar um "Quick Preview" ou descrição do crime na própria linha, se disponível.
